    ...
    <div className="min-h-screen bg-gradient-to-b from-[hsl(225,15%,6%)] to-[hsl(225,15%,4%)] relative">
      ...
      <div className="px-6 mb-8">
        <GlassmorphicCard className="p-6">
          <h2 className="text-xl font-bold mb-4">Your Wellness Journey</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-400">
                {activities?.length || 0}
              </div>
              <div className="text-sm text-white/70">Activities Logged</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-400">
                {activities?.filter(a => {
                  const today = new Date().toDateString();
                  return a.timestamp && new Date(a.timestamp).toDateString() === today;
                }).length || 0}
              </div>
          ...
          {user.primaryWellnessGoal && (
            <div className="mt-6 pt-4 border-t border-white/10">
              ...
              <p className="text-sm text-white/90">{user.primaryWellnessGoal}</p>
            </div>
          )}
        </GlassmorphicCard>
      </div>
